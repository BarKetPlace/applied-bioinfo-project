...
res_data_fnames <- list.files(path = dir_res, pattern = 'trim0.5.res', full.names = T, recursive = T)  # TODO: set to look for 'trim0.res' for now <-  need to change to 'trim10.res' later when TrimAl data is in 
...
#' Summarize statistics for each group (TrimAl)
stats <- res_df_all
...
all_summary <- rbind(statsm_summary, met_summary, notrim_summary)
